...
| -------------------- | ------------------------------ | ----------------------------- |
| **TAKE_PROFIT**      | \|Z\| <= Z_TP_THRESHOLD (0.0)  | Спред вернулся к среднему     |
| **STOP_LOSS**        | \|Z\| >= Z_SL_THRESHOLD (4.5)  | Спред разошёлся ещё сильнее   |
...
| **TIMEOUT**          | bars >= MAX_POSITION_BARS (96) | Позиция слишком долго открыта |
...
**Фильтры качества спреда (Screener Pipeline):**

Для каждого кандидата на вход применяется цепочка фильтров:

**1️⃣ Hurst Filter (Mean-Reversion Test):**

- Рассчитывается Hurst exponent для спреда за **HURST_LOOKBACK_CANDLES = 300 свечей**
- Если `H < HURST_THRESHOLD (0.45)` — спред mean-reverting, проходит дальше
- Если `H >= 0.45` — спред trending, вход запрещён

**2️⃣ Half-Life Filter (Mean-Reversion Speed):**

Измеряет скорость возврата к среднему через процесс Орнштейна-Уленбека:

```
Регрессия: diff = λ × lag + μ
Half-Life = -ln(2) / λ
```

- Если `λ >= 0` — спред не mean-reverting (random walk или diverging), возвращает ∞
- Правило: `Half-Life <= 0.5 × MAX_POSITION_BARS` (если держим 24ч, HL должен быть <= 12ч)
- **HALFLIFE_MAX_BARS = 48** (12 часов для 15m таймфрейма)
- **HALFLIFE_LOOKBACK_CANDLES = 300**

Комплементарен к Hurst: Hurst показывает _тенденцию_ к mean-reversion, Half-Life измеряет _скорость_.

**3️⃣ ADF Filter (Stationarity Test):**

Тест Augmented Dickey-Fuller на стационарность спреда:

- p-value < threshold → спред стационарен (отвергаем гипотезу о unit root) — ✅ GOOD
- p-value >= threshold → спред нестационарен — ❌ BAD
- **ADF_PVALUE_THRESHOLD = 0.08** (92% confidence)
- **ADF_LOOKBACK_CANDLES = 300**

Комплементарен к Hurst и Half-Life: ADF тестирует _статистическую стационарность_.
...
- **FALSE_ALARM_HYSTERESIS = 0.2** — отмена только если Z упал ниже порога на эту величину
...
При подтверждении разворота рассчитывается динамический TP:

```
z_tp_threshold = peak_Z × time_coef
```

Где `time_coef` зависит от времени в позиции (см. раздел Dynamic TP).
...
Чем дольше позиция открыта, тем агрессивнее TP. Формула:

```
dynamic_TP = peak_Z × time_coef
```

Где `peak_Z` — максимальный |Z| с момента входа, а `time_coef`:

| Время в позиции | Коэффициент |
| --------------- | ----------- |
| 0-4 часа        | 1.0         |
| 4-12 часов      | 3.0         |
| 12-24 часа      | 5.0         |
...
| Z_TP_THRESHOLD                 | 0.0       | Базовый порог TP                          |
| Z_SL_THRESHOLD                 | 4.5       | Порог SL                                  |
...
| HALFLIFE_MAX_BARS              | 48        | Макс. Half-Life (0.5 × MAX_POSITION_BARS) |
| HALFLIFE_LOOKBACK_CANDLES      | 300       | Окно для Half-Life расчёта                |
| ADF_PVALUE_THRESHOLD           | 0.08      | Макс. p-value для стационарности          |
| ADF_LOOKBACK_CANDLES           | 300       | Окно для ADF расчёта                      |
...
| FALSE_ALARM_HYSTERESIS         | 0.2       | Гистерезис для отмены false alarm         |
| POSITION_SIZE_USDT             | 100       | Размер позиции COIN leg                   |
| MAX_OPEN_SPREADS               | 5         | Максимум одновременных спредов            |
| COOLDOWN_BARS                  | 16        | Cooldown после неудачного выхода (~4h)    |
| MAX_POSITION_BARS              | 96        | Максимальная длительность позиции (~24h)  |
...
PlannerService (cron 15m)
    ...
    └─► OrchestratorService.run()
            ...
            │       └─► Filter Chain (for entry candidates):
            │               1️⃣ HurstFilter (H < 0.45)
            │               2️⃣ HalfLifeFilter (HL <= 48 bars)
            │               3️⃣ ADFFilter (p-value < 0.08)
            │
